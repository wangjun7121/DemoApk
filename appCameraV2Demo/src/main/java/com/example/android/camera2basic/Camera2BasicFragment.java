...
public class Camera2BasicFragment extends Fragment
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        ...
        // 设置 ImageButton 那个按钮的响应也为本函数
        view.findViewById(R.id.info).setOnClickListener(this);

        // 获得自定义的缩放的 TextureView
        mTextureView = (AutoFitTextureView) view.findViewById(R.id.texture);
    }

    @Override
    // 在 SD 卡的目录下建立 jpg 文件等待待将拍到的照片写进去
    ...
    public void onResume() {
        ...
        if (mTextureView.isAvailable()) {
            ...
            // 打开 Camera 进行显示
            //      openCamera() 里面，首先要配置相机的输出，预览图像和拍照的图片要作不同的处理，
            //          然后根据当前的设备屏幕环境，判断是否需要进行数据的转换，
            //          最后通过 cameraManager 打开摄像头（调用 cameraManager.openCamera() 方法）。
            openCamera(mTextureView.getWidth(), mTextureView.getHeight());
        } else {
            // 设置 TextureView 回调 SurfaceTextureListener
...
//        控制拍照的步骤大致如下。
//            调用 CameraManager 的 openCamera(String cameraId, CameraDevice.StateCallback callback, Handler handler) 方法打开指定摄像头。
//                该方法的第一个参数代表要打开的摄像头ID；
//                第二个参数用于监听摄像头的状态；
//                第三个参数代表执行callback的Handler，如果程序希望直接在当前线程中执行callback，则可将handler参数设为null。
//
//            当摄像头被打开之后，程序即可获取CameraDevice—即根据摄像头ID获取了指定摄像头设备，
...
//                第三个参数代表执行callback的Handler，如果程序希望直接在当前线程中执行callback，则可将handler参数设为null。
//
//            不管预览还是拍照，程序都调用 CameraDevice 的 createCaptureRequest(int templateType) 方法创建 CaptureRequest.Builder，
//            该方法支持 TEMPLATE_PREVIEW（预览）、TEMPLATE_RECORD（拍摄视频）、TEMPLATE_STILL_CAPTURE（拍照）等参数。
//            通过第 3 步所调用方法返回的 CaptureRequest.Builder 设置拍照的各种参数，比如对焦模式、曝光模式等。
//            调用 CaptureRequest.Builder的build() 方法即可得到 CaptureRequest 对象，接下来程序可通过 CameraCaptureSession 的 setRepeatingRequest() 方法开始预览，或调用 capture() 方法拍照。

        // 创建相机输出
        Activity activity = getActivity();

        // 获取 CameraMager 服务
        //      CameraDevice：描述一个照相机设备，一个Android设备可能会有多个摄像头，通过CameraId可以进行区别。它最主要有一个相机状态的
        //      回调函数，当下达打开相机的命令后，若相机正确的打开便会回调该函数
