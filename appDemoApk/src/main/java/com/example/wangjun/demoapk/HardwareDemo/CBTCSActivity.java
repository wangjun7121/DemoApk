...
public class CBTCSActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private CBTCS_RvAdapter mCBTCSRvAdapter;

    private RecyclerView mMessageView;
    private static CBTCS_MsgAdapter mMessageAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.hardwaredemo_cbtcs);
        // 这是我是为了 6.0 以上的设备能搜索到结果，动态申请了位置权限。但是没有处理结果，因为我测试肯定点同意~- -
    ...
    private void initUI() {
        ...
        // 设置【显示设备】的 RecyclerView 的 adapter 数据来源
        mCBTCSRvAdapter = new CBTCS_RvAdapter(this);
        mRecyclerView.setAdapter(mCBTCSRvAdapter);

        // 设置【显示设备】列表项点击响应函数
        mCBTCSRvAdapter.setOnItemClickListener(new CBTCS_RvAdapter.OnItemClickListener() {
            @Override
        ...
        // 设置【显示消息】的 RecyclerView 的 adapter 数据来源
        mMessageAdapter = new CBTCS_MsgAdapter(this);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.start:
                if (mBluetoothAdapter != null) {
                    mCBTCSRvAdapter.clearDevices();//开始搜索前清空上一次的列表
    ...
    class BlueToothStateReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            Toast.makeText(CBTCSActivity.this, "触发广播", Toast.LENGTH_SHORT).show();
            String action = intent.getAction();
            switch (action) {
                case BluetoothDevice.ACTION_FOUND:
                    BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
                    Toast.makeText(CBTCSActivity.this, "找到设备" + device.getName(), Toast.LENGTH_SHORT).show();
                    if (mCBTCSRvAdapter != null) {
                        mCBTCSRvAdapter.addDevice(device);
                    }
                    break;
